#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
RUYI-164 AIHub 每日精选 · 精选卡片生成脚本（1200×630）
输入: raw/ 下的官方渠道素材（来源与采集日期见 SOURCES.md）
输出: cards/*.png（6 张，原生分辨率，无放大）
设计体系: 深色统一版式 —— 左侧类型徽章/产品名/官方域名/一句话价值/License+Stars；
右侧官方真实产品视觉 + 来源图注；底部「AIHub 编辑精选」条。
星数与一句话价值均取自 RUYI-164 报批事实基线（快照 2026-09-21），未新增事实。
"""
import os
from PIL import Image, ImageDraw, ImageFont, ImageFilter

BASE = os.path.dirname(os.path.abspath(__file__))
RAW = os.path.join(BASE, 'raw')
OUT = os.path.join(BASE, 'cards')
os.makedirs(OUT, exist_ok=True)

W, H = 1200, 630
BG = (11, 14, 20)
INK = (230, 237, 243)
SUB = (139, 148, 158)
BODY = (201, 209, 217)

NOTO = '/usr/share/fonts/google-noto-cjk/NotoSansCJK-%s.ttc'
MONT = '/usr/share/fonts/julietaula-montserrat/Montserrat-%s.otf'


def noto_sc(weight, size):
    # 选出 ttc 中简体中文（SC）面的 index
    path = NOTO % weight
    for i in range(6):
        try:
            f = ImageFont.truetype(path, size, index=i)
            if 'SC' in f.getname()[0]:
                return f
        except Exception:
            break
    return ImageFont.truetype(path, size)


def font_cache(weight, path_cls):
    class C:
        def __call__(self, size):
            key = (path_cls, weight, size)
            if key not in _fc:
                _fc[key] = noto_sc(weight, size) if path_cls == 'n' else ImageFont.truetype(MONT % weight, size)
            return _fc[key]
    return C()


_fc = {}
F_BLACK = font_cache('Black', 'n')
F_BOLD = font_cache('Bold', 'n')
F_MED = font_cache('Medium', 'n')
F_REG = font_cache('Regular', 'n')
F_MONT_XB = font_cache('ExtraBold', 'm')
F_MONT_SB = font_cache('SemiBold', 'm')
F_MONT_MD = font_cache('Medium', 'm')


def tokenize(s):
    toks, buf, prev_cjk = [], '', False
    for ch in s:
        cjk = ord(ch) > 0x2E80
        if cjk or (prev_cjk and not ch.isalnum()):
            if buf:
                toks.append(buf)
            buf, prev_cjk = ch, cjk
        else:
            if len(buf) == 1 and buf in NO_LEAD:
                toks.append(buf)  # 标点独立成 token，禁则才能悬挂
                buf = ''
            buf += ch
            prev_cjk = False
    if buf:
        toks.append(buf)
    return toks


NO_LEAD = '、。」』）！？，'  # 行头禁则字符


def wrap(draw, text, font, maxw):
    lines, line = [], ''
    for t in tokenize(text):
        cand = line + t
        if draw.textlength(cand, font=font) <= maxw or not line:
            line = cand
        else:
            if len(t) == 1 and t in NO_LEAD:
                lines.append(line + t)  # 标点悬挂行尾，禁止行头
                line = ''
            else:
                lines.append(line)
                line = t.lstrip()
    if line:
        lines.append(line)
    return lines


def mix(c, bg, a):
    return tuple(int(bg[i] + (c[i] - bg[i]) * a) for i in range(3))


def glow(img, cx, cy, r, color, alpha):
    g = Image.radial_gradient('L').resize((r * 2, r * 2))
    g = g.point(lambda p: int((255 - p) * alpha))
    layer = Image.new('RGBA', img.size, (0, 0, 0, 0))
    tint = Image.new('RGBA', (r * 2, r * 2), color + (255,))
    tint.putalpha(g)
    layer.paste(tint, (int(cx - r), int(cy - r)), tint)
    img.alpha_composite(layer)


def fill_crop(im, tw, th, fx=0.5, fy=0.5):
    tr = tw / th
    w, h = im.size
    if w / h > tr:
        nw, nh = int(h * tr), h
    else:
        nw, nh = w, int(w / tr)
    x, y = int((w - nw) * fx), int((h - nh) * fy)
    return im.crop((x, y, x + nw, y + nh)).resize((tw, th), Image.LANCZOS)


def contain(im, tw, th, panel_bg):
    im = im.convert('RGB')
    im.thumbnail((tw, th), Image.LANCZOS)
    panel = Image.new('RGB', (tw, th), panel_bg)
    panel.paste(im, ((tw - im.width) // 2, (th - im.height) // 2))
    return panel


def rounded_mask(w, h, r):
    m = Image.new('L', (w, h), 0)
    ImageDraw.Draw(m).rounded_rectangle((0, 0, w - 1, h - 1), r, fill=255)
    return m


def chip(draw, x, y, text, font, dot=None, pad=14):
    tw = draw.textlength(text, font=font)
    extra = 16 if dot else 0
    hgt = 34
    draw.rounded_rectangle((x, y, x + tw + pad * 2 + extra, y + hgt), 17,
                           fill=mix((255, 255, 255), BG, 0.06), outline=(255, 255, 255, 26), width=1)
    cx = x + pad
    if dot:
        draw.ellipse((cx + 4, y + hgt // 2 - 4, cx + 12, y + hgt // 2 + 4), fill=dot)
        cx += extra
    draw.text((cx, y + 7), text, font=font, fill=INK)
    return x + tw + pad * 2 + extra


def card(item):
    acc = item['accent']
    img = Image.new('RGBA', (W, H), BG + (255,))
    glow(img, -80, -60, 560, acc, 0.16)
    glow(img, W + 60, H + 40, 640, acc, 0.10)
    d = ImageDraw.Draw(img)

    # ---- 左栏 ----
    lx, lw = 64, 492
    y = 66
    # 类型徽章 + 分类
    btxt = item['type']
    bf = F_MONT_SB(19)
    btw = d.textlength(btxt, font=bf)
    d.rounded_rectangle((lx, y, lx + btw + 30, y + 34), 17, fill=mix(acc, BG, 0.22),
                        outline=acc + (140,), width=1)
    d.text((lx + 15, y + 6), btxt, font=bf, fill=acc)
    d.text((lx + btw + 46, y + 7), item['cat'], font=F_MED(19), fill=SUB)
    y += 64

    # 产品名（自适应 / 两行）
    nf = F_BLACK(54)
    name = item['name']
    if d.textlength(name, font=nf) > lw:
        nf = F_BLACK(46)
    lines = wrap(d, name, nf, lw)
    for ln in lines:
        d.text((lx, y), ln, font=nf, fill=(255, 255, 255))
        y += int(nf.size * 1.22)
    y += 6

    # 官方域名/仓库
    df = F_MONT_SB(21)
    dom = item['domain']
    if d.textlength(dom, font=df) > lw:
        df = F_MONT_SB(17)
    d.text((lx, y), dom, font=df, fill=acc)
    y += int(df.size * 1.9)

    # 一句话价值（孤行回退：末行只剩 1–2 字时缩号重排）
    vf = F_MED(25)
    vlines = wrap(d, item['value'], vf, lw)
    while len(vlines) >= 2 and len(vlines[-1]) <= 2 and vf.size > 21:
        vf = F_MED(int(vf.size) - 2)
        vlines = wrap(d, item['value'], vf, lw)
    vy = y + 8
    for ln in vlines[:4]:
        d.text((lx, vy), ln, font=vf, fill=BODY)
        vy += 38

    # 分隔线 + chips
    d.line((lx, 512, lx + lw, 512), fill=(255, 255, 255, 22), width=1)
    cf = F_MED(16)
    cfx = F_MED(16)
    x = lx
    x = chip(d, x, 530, item['license'], cfx, dot=acc) + 12
    x = chip(d, x, 530, '★ ' + item['stars'], cf) + 12
    chip(d, x, 530, '快照 2026-09-21', cfx)

    # ---- 右栏视觉 ----
    px, py, pw, ph, rad = 592, 56, 544, 452, 18
    if item['fit'] == 'contain':
        panel = contain(Image.open(os.path.join(RAW, item['img'])), pw, ph, item.get('panel', (245, 246, 248)))
    else:
        panel = fill_crop(Image.open(os.path.join(RAW, item['img'])).convert('RGB'), pw, ph,
                          item.get('fx', .5), item.get('fy', .5))
    img.paste(panel, (px, py), rounded_mask(pw, ph, rad))
    d.rounded_rectangle((px, py, px + pw, py + ph), rad, outline=(255, 255, 255, 28), width=1)

    # OrcaReplay 官方 logo 小徽标（白底圆角片，右下角）
    if item.get('inset'):
        li = Image.open(os.path.join(RAW, item['inset'])).convert('RGBA')
        li.thumbnail((150, 46), Image.LANCZOS)
        pad = 12
        bw, bh = li.width + pad * 2, li.height + pad * 2
        bx, by = px + pw - bw - 14, py + 14
        plate = Image.new('RGBA', (bw, bh), (255, 255, 255, 235))
        img.paste(plate, (bx, by), rounded_mask(bw, bh, 12))
        img.paste(li, (bx + pad, by + pad), li)
        d.rounded_rectangle((bx, by, bx + bw, by + bh), 12, outline=(255, 255, 255, 40), width=1)

    # 来源图注（面板内底部渐变条）
    strip_h = 40
    grad = Image.new('L', (1, strip_h))
    for i in range(strip_h):
        grad.putpixel((0, i), int(210 * (i / (strip_h - 1)) ** 1.5))
    grad = grad.resize((pw, strip_h))
    black = Image.new('RGBA', (pw, strip_h), (0, 0, 0, 255))
    black.putalpha(grad)
    img.paste(black, (px, py + ph - strip_h), black)
    d = ImageDraw.Draw(img)
    d.text((px + 16, py + ph - strip_h + 10), item['caption'], font=F_REG(15), fill=(235, 240, 245))

    # ---- 底部条 ----
    d.line((0, 578, W, 578), fill=acc + (120,), width=2)
    d.text((64, 592), 'AIHub 编辑精选 · 每日 AI 新资源（2026-09-21 期）', font=F_MED(17), fill=SUB)
    rt = 'FEATURED · AIHUB'
    rf = F_MONT_SB(15)
    d.text((W - 64 - d.textlength(rt, font=rf), 594), rt, font=rf, fill=mix(acc, BG, 0.75))

    img.convert('RGB').save(os.path.join(OUT, item['out']), 'PNG')
    print('written cards/' + item['out'])


ITEMS = [
    dict(out='01_taste-skill.png', type='SKILL', cat='前端与设计', name='Taste Skill',
         domain='tasteskill.dev', accent=(201, 162, 107),
         value='给 AI agent「好品味」的反 slop 前端技能集，阻止生成平庸通用的界面',
         license='MIT', stars='88,767', img='official_tasteskill_og-image.jpg', fit='fill', fx=.07, fy=.5,
         caption='视觉来源：tasteskill.dev 官方 OG 图 · 采集 2026-09-22'),
    dict(out='02_world-monitor.png', type='APP', cat='情报聚合仪表盘', name='World Monitor',
         domain='worldmonitor.app', accent=(47, 213, 117),
         value='实时全球情报仪表盘：AI 新闻聚合、地缘监测、基础设施追踪一屏呈现',
         license='AGPL-3.0', stars='87,067', img='official_worldmonitor_world-variant.png',
         fit='fill', fy=.32, caption='视觉来源：github.com/koala73/worldmonitor 官方截图 · 采集 2026-09-22'),
    dict(out='03_appllama-skills.png', type='SKILL', cat='移动应用构建', name='Appllama Skills',
         domain='appllama.io', accent=(233, 200, 143),
         value='对照头部畅销应用研究出的 agent 技能集，让 agent 真正会做移动 App',
         license='MIT', stars='1,914', img='official_appllama_og.png', fit='fill', fy=.52,
         caption='视觉来源：appllama.io 官方 OG 图 · 采集 2026-09-22'),
    dict(out='04_ai-data-extractor.png', type='APP', cat='开发者数据工具', name='AI Data Extractor',
         domain='github.com/kruzovic7/ai-data-extractor', accent=(88, 166, 255),
         value='一键提取 Claude Code、Cursor、Windsurf、Aider、Cline/Roo 等聊天历史',
         license='MIT', stars='842', img='gh_social_kruzovic7_ai-data-extractor.png',
         fit='contain', panel=(246, 247, 249),
         caption='视觉来源：GitHub 官方 Social Preview 渲染 · 采集 2026-09-22'),
    dict(out='05_orcareplay.png', type='APP', cat='agent 调试回放', name='OrcaReplay',
         domain='github.com/Continuum-AI-Corp/OrcaReplay', accent=(76, 141, 255),
         value='给 agent「时间旅行」：录制、回放、分叉、调试任意模型驱动的 agent 运行',
         license='Apache-2.0', stars='258', img='official_orcareplay_graph-card.png',
         fit='fill', fx=.5, fy=.5, inset='official_orcareplay_logo.png',
         caption='视觉来源：OrcaReplay 官方仓库运行图谱卡片与 Logo · 采集 2026-09-22'),
    dict(out='06_gap-trap.png', type='SKILL', cat='工程质量门控', name='Gap Trap',
         domain='pliablepixels.github.io/gap-trap', accent=(124, 108, 240),
         value='仓库内规则与门控让 AI 代码保持正确，把 vibe coding 变成高质量代码',
         license='MIT', stars='176', img='official_gaptrap_logo_assets.png',
         fit='contain', panel=(19, 26, 51),
         caption='视觉来源：gap-trap 官方仓库 Logo · 采集 2026-09-22'),
]

if __name__ == '__main__':
    for it in ITEMS:
        card(it)
