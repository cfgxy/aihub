...
export const resourceProfiles: Record<string, ResourceProfile> = {
  ...
  "mirofish": {
    image: "/media/mirofish.png",
    imageAlt: "MiroFish 精选卡片：官方五阶段工作流示意配深色版式，左侧 APP 徽章、一句话价值与 AGPL-3.0 许可证",
    imageCredit: "卡片：AIHub 编辑制作（视觉素材来自各产品官方渠道）",
    overview: [
      "MiroFish 是一个开源群体智能引擎，定位为「简洁通用的群体智能引擎，预测万物」（项目方口径）。工作方式是：从现实世界提取种子信息（突发新闻、政策草案、金融信号等），自动构建高保真的并行数字世界，其中数千个具备独立人格、长期记忆和行为逻辑的智能体自由交互并经历社会演化；用户可以「上帝视角」动态注入变量，观察系统推演未来轨迹，最终得到一份详细的预测报告和一个可继续深度交互的数字世界。宏观上它被项目方定位为决策者的零风险演练实验室（政策推演、公关推演），微观上也可作为个人创意沙盒，例如推演一部小说的结局。",
      "技术实现上，MiroFish 的仿真流水线分为五阶段：图构建（种子提取、个体/集体记忆注入、GraphRAG 知识图谱）→ 环境搭建（实体关系抽取与智能体人格生成）→ 并行仿真 → 报告生成（ReportAgent 智能体）→ 深度交互（与仿真世界中任意智能体对话）。仿真引擎基于 CAMEL-AI 团队的开源项目 OASIS，项目获盛大集团战略支持与孵化；支持任何 OpenAI SDK 兼容的 LLM API，提供源码部署与 Docker 两种部署方式。须知：「预测万物」为项目宣传口径，预测准确性无独立第三方验证，输出不可作为投资或重大决策依据；AGPL-3.0 对闭源商用集成有传染性约束；运行需自备 LLM API Key（官方推荐阿里云百炼 qwen-plus）与 Zep Cloud 记忆服务（有免费月度额度），仿真 token 消耗较高（README 建议先跑 40 轮以内）；仓库最近推送 2026-09-03，活跃度中等；星数 73,020 为 2026-09-15 GitHub API 快照值。",
    ],
    highlights: [
      "多智能体社会仿真：从种子材料自动构建并行数字世界，数千智能体带独立人格与长期记忆社会演化",
      "上帝视角变量推演：动态注入变量干预仿真进程，推演不同条件下的未来轨迹",
      "预测报告与深度交互：自动生成结构化预测报告，并可与仿真世界内任意智能体继续对话追问",
    ],
    bestFor: "研究群体智能方法、需要多智能体预测聚合的开发者与分析师。",
  },
  "yue": {
    image: "/media/yue.png",
    ...
    imageCredit: "卡片：AIHub 编辑制作（视觉素材来自各产品官方渠道）",
    overview: [
      "YuE2 是统一符号与音频两种模态的开源音乐生成模型：用户输入歌词和风格提示词，模型先生成旋律与和弦的符号规划（symbolic plan），再将规划渲染为带人声与伴奏的完整歌曲（48 kHz 立体声）。与「黑盒直出」不同，这条「先规划、后渲染」的流水线让旋律和和弦成为显式控制项，可以在渲染前被人工或智能体检查、修改。项目方口径称其在 WildSongBench 上与 Suno v5/v6 具有竞争力（YuE2 best-of-8 达 SongBench 均值 6.9632，为项目方评测数据，未经独立验证）。",
    ...
    highlights: [
      "白盒符号规划：旋律与和弦先于音频显式生成，渲染前可人工编辑或交由智能体修改",
      "零样本翻唱：通过 SheetSage2 转录源录音，用新歌词或目标风格重新演绎，无需翻唱专用微调",
      "Agentic 音乐编辑：通过对话迭代修改乐谱、编曲与歌词（README 示例：一首普通话流行歌经 9 步 14 个版本改为英文爵士）",
    ],
    bestFor: "游戏/视频配乐创作者与音乐 AI 研究者。",
  },
  "flowsint": {
    image: "/media/flowsint.png",
    imageAlt: "flowsint 精选卡片：图谱实体与丰富器示意配深色版式，左侧 APP 徽章、一句话价值与 Apache-2.0 许可证",
    imageCredit: "卡片：AIHub 编辑制作（视觉素材来自各产品官方渠道）",
    overview: [
      "flowsint 是一个面向道德调查、透明与验证场景的开源图谱调查平台（README 原文：designed for ethical investigation, transparency, and verification），为网络安全分析师与调查人员设计。它通过可视化图谱界面组织调查对象——域名、IP、邮箱、社交账号、加密货币钱包等实体——并以自动化 enrichers 批量扩展实体关系：反查 DNS 与子域名枚举、WHOIS 与历史记录、用户名跨平台搜索（Maigret）、钱包交易与 NFT 查询、网站爬虫与追踪器识别、邮箱与电话的数据泄露查询等；界面宣称在数千节点规模下依然流畅。",
      "工程上，flowsint 采用 Python/FastAPI + Neo4j 图数据库 + PostgreSQL/Redis/Celery 的技术栈，提供 Docker Compose 部署与 GitHub Container Registry 预构建镜像，支持局域网/服务器部署供团队协作，内置实时事件流（FastAPI）与 N8n 集成。数据架构强调「一切存储在你的机器上」（Everything is stored on your machine），适合对数据驻留敏感的调查团队。须知：调查能力天然涉及个人信息与第三方数据查询，数据合规使用责任在使用方；README 明确禁止用于未授权入侵、监控、骚扰、doxxing、政治操纵等场景，并要求遵循其 ETHICS.md 负责任使用指南；活跃度中等（最近推送 2026-09-06）；星数 8,344 为 2026-09-15 GitHub API 快照值。",
    ],
    highlights: [
      "可视化关系图谱：以图谱界面组织多源实体与关系，数千节点规模保持流畅操作",
      "自动化情报丰富器：域名/IP/社交/加密货币/网站/邮箱/电话等类别的批量情报扩展，支持 N8n 工作流集成",
      "本地自托管与团队协作：数据全部存储于本机，支持局域网/服务器部署与实时事件流",
    ],
    bestFor: "情报分析、尽调与安全研究人员——README 明确列举：网络安全研究人员与分析师、记者与 OSINT 调查人员、执法或欺诈调查团队、开展内部威胁情报或数字风险分析的组织。",
  },
  "tradingagents": {
    image: "/media/tradingagents.png",
    imageAlt: "TradingAgents 精选卡片：多智能体角色链示意与合规警示条配深色版式，左侧 APP 徽章、一句话价值与研究用途定位",
    imageCredit: "卡片：AIHub 编辑制作（视觉素材来自各产品官方渠道）",
    overview: [
      "TradingAgents 是一个模拟真实交易公司运作的多智能体交易分析框架（README 原文：mirrors the dynamics of real-world trading firms）。其决策链还原了交易机构的组织分工：分析师团队（基本面、情绪、新闻、技术四类分析师分别评估公司财务、聚合 StockTwits/Reddit 情绪、监测全球新闻与宏观指标、以 MACD/RSI 等指标识别形态）产出研究报告；「看多 vs 看空」研究员通过结构化辩论权衡收益与风险；交易员智能体汇总报告决定交易时机与规模；风控与投资组合经理持续评估波动率与流动性，批准后订单发送至模拟交易所执行。",
      "框架基于 LangGraph 构建，提供交互式 CLI 与 Python API 两种入口；每次运行的决策追加至本地交易记忆文件，下次分析同一标的时注入已实现收益与反思，支持检查点续跑。市场覆盖美股、港股、中国 A 股、东京、伦敦、印度、加拿大、澳洲及加密货币（行情基于 Yahoo Finance 等），LLM 支持面广（OpenAI、Gemini、Claude、Grok、DeepSeek、Qwen、GLM、MiniMax、Ollama 本地模型、Azure/Bedrock 及任意 OpenAI 兼容端点）。项目附论文（arXiv:2412.20138）与 Trading-R1 技术报告（arXiv:2509.11420），研究背景完整。须知：涉资金域，框架定位为研究用途（designed for research purposes），输出不可作投资依据——README 原文声明「It is not intended as financial, investment, or trading advice」；交易表现受骨干模型、温度、分析时段与数据质量等因素影响，回测结果不保证与任何已发表数字相符；订单仅在模拟交易所执行，非实盘交易；LLM API 与部分数据源费用由用户自备，使用 Ollama 本地模型可零 API 成本；星数 106,136 为 2026-09-15 GitHub API 快照值。",
    ],
    highlights: [
      "全链路多智能体角色模拟：分析 → 多空辩论 → 交易决策 → 风控审批的完整机构式决策链，智能体动态讨论收敛结论",
      "多市场多数据源：覆盖美股/A股/港股/加密货币等多个市场，聚合行情、情绪、新闻多类数据源",
      "决策记忆与可复现研究：交易记忆持久化并在后续分析中注入历史反思，LangGraph 检查点支持续跑，模块化便于研究改造",
    ],
    bestFor: "量化/金融 AI 研究者与多智能体系统研究者。",
  },
};
