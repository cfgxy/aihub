...
export default async function ResourcePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <SiteShell><div className="container detail-page">
    ...
    {profile && <figure className="detail-visual"><img src={profile.image} alt={profile.imageAlt}/><figcaption>{profile.imageSource
      ? <>图片来源：<a href={profile.imageSource} target="_blank" rel="noopener nofollow">官方页面 / 来源仓库</a></>
      : profile.imageCredit}</figcaption></figure>}
    <div className="detail-layout"><div><AcquisitionPanel resource={resource}/><section className="detail-copy"><span className="section-kicker">资源简介</span><h2>关于 {resource.name}</h2>{profile ? profile.overview.map((paragraph) => <p key={paragraph}>{paragraph}</p>) : <p>{resource.description || resource.summary}</p>}
      {profile?.featureImage && <figure className="detail-feature"><img src={profile.featureImage} alt={profile.featureImageAlt}/><figcaption>{profile.imageCredit}</figcaption></figure>}
